...
export type LeadFormKind = 'player' | 'creator';

const KIND_LABELS: Record<LeadFormKind, string> = {
	player: 'Заявка гравця',
	creator: 'Заявка творця',
};
...
export class LeadFormComponent {
	...
	protected async submit(form: NgForm): Promise<void> {
		...
		const lines = [
			`Play lead: ${KIND_LABELS[this.kind()]}`,
			`Ім'я: ${this.name()}`,
		...
		if (!success) { this.status.set('error'); return; }
		this.status.set('success');
		form.resetForm();
	}

	protected retry(): void { this.status.set('idle'); }
}
